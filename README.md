...
- **Structured AI Outputs**: Itineraries generated by AI use Anthropic Claude tool calling (`generate_itinerary`) to enforce output parsing into a typed schema.
- **Open-Meteo Integration**: Connects to `open-meteo.com` to dynamically fetch live weather and coordinates for destination cities to optimize suggestions.
- **Pydantic Validation**: Converts and validates generated itineraries against `AIItinerarySchema` to detect formatting anomalies and constraint violations.
- **Agentic Correction Loop**: If the LLM output fails schema validation, budget checks, day counts, or destination matches, the system sends the errors back to the LLM in a multi-turn session to correct itself (up to 3 attempts, with 2 retries).
...
- **Prompt Caching**: Implements Anthropic prompt caching boundaries using `cache_control` to reduce latency and token costs on the retrieved document context.
- **CDC Manifest Tracking**: Uses a database-backed `DocumentManifest` table to tracking hash changes on documents, preventing redundant indexing and API uploads.
